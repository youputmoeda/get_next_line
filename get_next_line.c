...
/*   Updated: 2022/02/03 20:35:04 by joteixei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (!buffer)
	{
		...
		while (line_text == NULL && i > 0)
		{
			...
			line_text = gnl_main(&var_local, &buffer, i);
		}
		free(buffer);
	}
	else
		line_text = gnl_get(&var_local, (buffer - var_local) + 1);
	return (line_text);
}
